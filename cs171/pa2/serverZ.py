...
def requestThread(host, cPort,connect, ip, port, userInput, bPort, ):
	global total	
	config.currentStamp = config.timestamp + 1
	print("Transaction received from C (Logical time:" + str(config.timestamp) + ")")
	config.timestamp = config.timestamp + 1
	...
	msg = str(bPort)
	print("REQUEST <" + str(config.currentStamp) + ",3> sent to all")
	sc.sendall(msg.encode('utf-8'))
	config.timestamp = config.timestamp + 2 
	msg = sc.recv(2048).decode('utf-8')
	if msg == 'REPLY':
		config.timestamp = config.timestamp + 1
		print("REPLY to <"+ str(config.currentStamp) + ",3> received from server x (Logical time: " + str(config.timestamp) + ")")
		config.timestamp = config.timestamp + 1
		print("REPLY to <"+ str(config.currentStamp) + ",3> received from server y (Logical time: " + str(config.timestamp) + ")")
		config.timestamp = config.timestamp + 1
		print("Run critical section for <" + str(config.currentStamp) + ",3> (Logical time: " + str(config.timestamp) + ")")
		f.write(userInput)
		total = total + float(clientInput[4:])
	msg = "RELEASE"
	config.timestamp = config.timestamp + 1
	sc.sendall(msg.encode('utf-8'))
	print("RELEASE for <" + str(config.currentStamp) + ",3> sent to all (Logical time: " + str(config.timestamp) + ")")
	connect.sendall(clientInput.encode('utf-8'))
	f.close
def main():
	host = "127.0.0.1"
	bPort = 7777 #client port
	...
	print("Server z started (Logical time:" + str(config.timestamp) + ")")
